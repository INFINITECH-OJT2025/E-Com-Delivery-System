...
class RestaurantController extends Controller
{
    ...
    public function show($slug)
    {
        $restaurant = Restaurant::where('slug', $slug)
            ->with([
                'menus:id,restaurant_id,menu_category_id,name,price,image,availability,description', // ✅ Added 'description'
        ...
        $bestSellers = $restaurant->menus()
            ->select('menus.id', 'menus.restaurant_id', 'menus.menu_category_id', 'menus.name', 'menus.price', 'menus.image', 'menus.availability', 'menus.description') // ✅ Added 'description'
            ->leftJoin('order_items', 'menus.id', '=', 'order_items.menu_id') // ✅ Count menu items from orders
            ->groupBy('menus.id', 'menus.restaurant_id', 'menus.menu_category_id', 'menus.name', 'menus.price', 'menus.image', 'menus.availability', 'menus.description') // ✅ Group by 'description' too
            ->orderByRaw('COUNT(order_items.id) DESC') // ✅ Most ordered items first
            ->limit(5)
            ->get();

        // ✅ Extract unique menu categories **for this restaurant only**
        $menuCategories = $restaurant->menus->pluck('category')->unique()->values();

        // ✅ Add best sellers inside the restaurant object
